...
class FileSystem(object):
	def __init__(self, param):
		self.__repo_dir = os.path.expanduser(param.get('dir', '~/jsub/task_info'))
	...
	def save_task(self, data):
		...
		safe_mkdir(os.path.join(self.__repo_dir,str(data['id'])))
		task_path = os.path.join(self.__repo_dir, str(data['id']),'repo')
	...
	def delete_task(self, task_id):
		safe_rmdir(os.path.join(self.__repo_dir,str(task_id)))
	...
	def all_task_data(self, order='asc'):
		task_ids =[d for d in os.listdir(self.__repo_dir) if os.path.isdir(os.path.join(self.__repo_dir,d))]
		task_ids.sort(key=int, reverse=(order=='desc'))
		return self.find_by_ids(task_ids)

	def task_data(self, task_id):
		safe_mkdir(os.path.join(self.__repo_dir,str(task_id)))
		task_path = os.path.join(self.__repo_dir,str(task_id),'repo')
		with open(task_path, 'a+') as f:
			fcntl.flock(f, fcntl.LOCK_EX)
			f.seek(0)
			data_str = f.read()
	...
	def __new_task_id(self):
		task_ids =[int(d) for d in os.listdir(self.__repo_dir) if os.path.isdir(os.path.join(self.__repo_dir,d))]
		if not task_ids:
			return 1
