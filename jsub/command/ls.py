...
COLUMN_TITLE = {
	...
	'scenario':		'Experiment',
	'backend':	'Backend',
	'status':	 'Status (D|F|R|W|O)',
	'created_at': 'Creation Time (UTC)',
	'updated_at': 'Info Updated (UTC)',
}
...
class Ls(object):
	def __init__(self, jsubrc, task_ids, update = False):
	...
	def execute(self):
		...
		tasks_data = j.ls(self.__task_ids, self.__update)

		columns = ['id', 'name', 'scenario', 'backend', 'status', 'created_at','updated_at']
